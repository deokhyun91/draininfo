...
export default function Board_W(){
    ...
    <form onSubmit={onSubmit} className='joinWrap wrap_in'>
        ...
        <div className="input_area">
           ...
           <input type="text" placeholder="닉네임" ref={nameRef} disabled={!checked}/>
        </div>
        <div className="input_area">
           
           <input type="text" placeholder="제목"  ref={titleRef} required/>
       </div>
       <div className="input_area">
            <span className="fs15 checknum">현재 글자 수 : {inputCount}</span>
           <textarea onChange={onInputHandler} placeholder="내용을 입력해 주세요(200자이하)" maxLength="200"  ref={contentRef} required />
       </div>
        <button
        style={
            {opacity: isLoading ? 0.3 : 1}
        } className='joinBtn'
        >{isLoading ? "Saving..." : "작성완료"}</button>
     </form>
    </motion.div>
      )
}